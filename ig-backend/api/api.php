...
class PHP_CRUD_API {
	...
	protected function parseRequestParameter(&$request,$characters, $strip=true) {
		if ($request==='') return false;
		$pos = strpos($request,'/');
		$value = $pos?substr($request,0,$pos):$request;
		if ($strip) $request = $pos?substr($request,$pos+1):'';
    ...
    protected function getTableList($database, $table) {
	...
	protected function processTableAndIncludeParameters($database,$table,$include,$action) {
        $table_list = $this->getTableList($database, $table);
...
//		}
		if (!$this->settings['request']) {
            ...
            if (!empty($tableList)) {
                ...
                while(!file_exists($request.'/index.php') && $request !=='') {
                    $pos = strrpos($request,'/');
